    ...
    public void onSubmit(FormPanel.SubmitEvent event) {
        ...
        if (token != null) {
            TaskDto taskDto = new TaskDto(titleText.getValue(),
                    ...
                    new UserDto(Long.parseLong(ownerList.getSelectedValue())),
            ...
            tasksClient.createTask(token, taskDto, new MethodCallback<Void>() {
                @Override
                public void onFailure(Method method, Throwable throwable) {
                    //нужно кинуть на страницу авторизации
                    GWT.log(throwable.toString());
                    GWT.log(throwable.getMessage());
                    Window.alert("Невозможно сохранить задачу: Сервер не отвечает");
                }

                @Override
                public void onSuccess(Method method, Void result) {
                    GWT.log("Created task");
